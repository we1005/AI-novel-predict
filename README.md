...
### 🌿 多大纲分支 · 记忆隔离与逐章回滚（2026-07 新增）
一本书往往能推演出**多条到结局的故事弧候选**（如「剑灵泣血·人间棋局」vs「道源之劫」）。想沿不同弧**并行续写**又不互相污染——**「分支即派生书」**：一条候选弧 = 一本独立 `novel.db`，从原著克隆 `ch1..N` 记忆作基线，续写只写进自己库。

- **天然隔离**：各分支的成稿 + 「写→回灌」抽取的实体/伏笔互不串味（复用墨笔"每本书独立 sqlite + `book_scope`"既有资产，几乎零核心改动）。
- **一键建分支**：书架「建分支」或整本故事弧推演面板「🌿 把此候选弧建为分支」。
- **逐章回滚**：`reset_continuation(from)` 删 `chapter≥from` 的全部续写产物(草稿+回灌实体+伏笔回收点+登记章)，原著不动 → 状态自动回到上一章末。
- **三层切换**：书架「切换到本书」(换工作书·可写) / 「建分支」/ 图谱·宝物·疑点页「视角」下拉(只读并排对比)。

> 详见 [`10 · 多大纲分支与记忆隔离`](墨笔-agent架构设计docs/10-多大纲分支与记忆隔离.md)。
...
- [09 · 笔法片段库与拆解](墨笔-agent架构设计docs/09-笔法片段库与拆解.md)
- [10 · 多大纲分支与记忆隔离](墨笔-agent架构设计docs/10-多大纲分支与记忆隔离.md) 🆕
